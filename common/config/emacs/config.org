...
This is the main configuration, but there also exists one more init file, ~init.el~ , that bootstraps ~org-mode~ before this file is interpreted. That's defined [[https://github.com/dustinlyons/nixos-config/blob/main/common/files.nix#L5][here]].
...
#+BEGIN_SRC emacs-lisp
  (setq user-full-name "Maxence Maireaux"
    user-mail-address "maxence@maireaux.fr")
#+END_SRC
  ...
  (if (daemonp)
      (add-hook 'after-make-frame-functions
                (lambda (frame)
                  ...
                  ;; Load other GUI-related configurations here
                  ))
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Set path for darwin
  (when (system-is-mac)
    (setenv "PATH" (concat (getenv "PATH") ":/Users/flemzord/.nix-profile/bin:/usr/bin"))
    (setq exec-path (append '("/Users/flemzord/bin" "/profile/bin" "/Users/flemzord/.npm-packages/bin" "/Users/flemzord/.nix-profile/bin" "/nix/var/nix/profiles/default/bin" "/usr/local/bin" "/usr/bin") exec-path)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-agenda-hide-tags-regexp (regexp-opt '("Todo" "home" "work" "batch")))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun dl/load-buffer-with-emacs-config ()
    ...
    (interactive)
    (dl/find-file "~/.local/share/src/nixos-config/common/config/emacs/config.org"))
  ...
  (defun dl/load-buffer-with-nix-config ()
    "Open the emacs configuration"
    (interactive)
    (dl/find-file "~/.local/share/src/nixos-config/common/home-manager.nix"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package projectile
    ...
    :init
      ...
      (when (system-is-linux)
        (setq projectile-project-search-path '("/home/flemzord/.local/share/src/good-data-lol")))
      (when (system-is-mac)
        (setq projectile-project-search-path '("/Users/flemzord/.local/share/src/good-data-lol")))
