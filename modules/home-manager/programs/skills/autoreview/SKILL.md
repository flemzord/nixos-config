...
```bash
<autoreview-helper> --reviewers codex,claude --model codex=gpt-5.1 --model claude=sonnet --thinking codex=high --thinking claude=max
```
...
This skill bundles the helper under its own `scripts/` directory. Invoke it via
...
```powershell
python ~\.claude\skills\autoreview\scripts\autoreview --help
```

The smoke harness has thin shell wrappers over a shared Python implementation:

```bash
~/.claude/skills/autoreview/scripts/test-review-harness --fixture benign --engine codex
```

```powershell
~\.claude\skills\autoreview\scripts\test-review-harness.ps1 -Fixture benign -Engine codex
```
